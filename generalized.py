...
def run_debate(number_of_rounds, chat_history_A, chat_history_B):
    for round_num in range(number_of_rounds):
        ...
        outputs_A = model_A.generate(input_ids = inputs, max_new_tokens=160, do_sample = True, temperature = .8)
        ...
        outputs_B = model_A.generate(input_ids = inputs, max_new_tokens=160, do_sample = True, temperature = .8)
        ...
        cleaned_answer_A = parse_final_answer_correctly(tokenizer.decode(extract_text_A_outputs[0], skip_special_tokens=True))
        cleaned_answer_B = parse_final_answer_correctly(tokenizer.decode(extract_text_B_outputs[0], skip_special_tokens=True))
