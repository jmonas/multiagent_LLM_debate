...
def run_debate(number_of_rounds, number_of_agents, temperatures):
    ...
    for round_num in range(number_of_rounds):
        ...
        for i in range(number_of_agents):
            ...
            outputs = model.generate(input_ids = inputs, max_new_tokens=125, do_sample = True, temperature = temperatures[i])
...
temperatures = [0.2, 0.8, 0.8]
current_time = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
json_file_path = f'results/debate_results_leader_{number_of_agents}_{num_rounds}_{current_time}_{str(temperatures[0])}.json'

print(f"{num_debates}  - {number_of_agents} -  {num_rounds}   - {str(temperatures[0])}", flush=True)
for debate_round in range(num_debates):
    print("START", flush=True)
    start_time = time.time()
    expression, truth, answers = run_debate(num_rounds, number_of_agents, temperatures)
